/* Enigma IC attack - OpenMP multi-core version
...
int main(void) {
    ...
    #pragma omp parallel for schedule(dynamic) reduction(+:total_hits)
    for (int ord = 0; ord < nord; ord++) {
        int tl = orderings[ord][0];
        int tm = orderings[ord][1];
        int tr = orderings[ord][2];
    ...
    printf("Candidates: 5905536\n");
    printf("Hits (IC >= %d): %d\n", threshold, total_hits);
